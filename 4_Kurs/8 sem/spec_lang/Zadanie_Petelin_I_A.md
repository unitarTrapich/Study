...
---
### 3.1 Краткое описание ролей на use case-диаграмме

- **Гость** — неавторизованный посетитель системы. Может зарегистрироваться и выполнить вход.
- **Пользователь** — авторизованный пользователь (например, студент или преподаватель). Загружает материалы, запускает извлечение знаний, просматривает граф, выполняет поиск и экспортирует результаты.
- **Администратор** — пользователь с расширенными правами. Управляет учетными записями, модерирует материалы и запускает повторную обработку.
- **NLP/ML сервис** — внешний интеллектуальный сервис, который извлекает сущности и связи из учебных материалов при обработке.
- 
